...
public class GroundShapes
{
  ...
  public GroundShapes(int floorLen, GL2 gl)
  { 
    generatePosns(floorLen);
    shapeTex = loadTexture(TEX_FNM, gl);
  } // end of GroundShapes()
  ...
  private Texture loadTexture(String fnm, GL2 gl)
  {
...
*/
     ...
     Each image is layered over a quad as a texture. */
  {
    ...
    for (int i=0; i < NUM_SHAPES; i++) {
      if (visibles[i]) {
        ...
        verts[0] = xc-SH_SIZE/2; verts[1] = 0.1f;  verts[2] = zc+SH_SIZE/2;
        verts[3] = xc+SH_SIZE/2; verts[4] = 0.1f;  verts[5] = zc+SH_SIZE/2;
        verts[6] = xc+SH_SIZE/2; verts[7] = 0.1f;  verts[8] = zc-SH_SIZE/2;
        verts[9] = xc-SH_SIZE/2; verts[10] = 0.1f; verts[11] = zc-SH_SIZE/2;
  ...
  private void drawScreen(GL2 gl, float[] verts, Texture tex)
