...
public class TourCanvasGL extends Canvas implements Runnable
{
  ...
  {
    xCamPos = 0; yCamPos = 1; zCamPos =  Z_POS;    // camera posn
     ...
     in GLEventListener) */
  {
    ...
    loadTextures();
    addLight();
    groundShapes = new GroundShapes(FLOOR_LEN, gl);
  ...
  private void loadTextures()
  {
    ...
    // rTex.setTexParameteri(GL2.GL_TEXTURE_WRAP_S, GL2.GL_REPEAT);
    // rTex.setTexParameteri(GL2.GL_TEXTURE_WRAP_T, GL2.GL_REPEAT);
  }  // end of loadTextures()
  ...
  */
  {
    ...
    if (gl.glIsEnabled(GL2.GL_LIGHTING)) {   // switch lights off if currently on
  ...
  // draw the earth orbiting a point, and rotating around its y-axis
  {
    ...
    gl.glMaterialfv(GL2.GL_FRONT, GL2.GL_SPECULAR, whiteCol, 0);
    gl.glMateriali(GL2.GL_FRONT, GL2.GL_SHININESS, 100);
  ...
  */
  {
    ...
    gl.glBegin(GL2.GL_QUADS);
      ...
      gl.glTexCoord2f(left, bottom); gl.glVertex3i(-edge, 0, -edge);
      gl.glTexCoord2f(2*right, bottom); gl.glVertex3i(edge, 0, -edge);
      gl.glTexCoord2f(2*right, top); gl.glVertex3i(edge, edge, -edge);
      gl.glTexCoord2f(left, top); gl.glVertex3i(-edge, edge, -edge);
      ...
      // ceiling
      gl.glTexCoord2f(left, bottom); gl.glVertex3i(edge, edge, edge);
      gl.glTexCoord2f(2*right, bottom); gl.glVertex3i(-edge, edge, edge);
      gl.glTexCoord2f(2*right, 2*top); gl.glVertex3i(-edge, edge, -edge);
      gl.glTexCoord2f(left, 2*top); gl.glVertex3i(edge, edge, -edge);
    gl.glEnd();
